...
\begin{center}
 ...
 \end{math} 
    ...
    Therefore, \(G(j\omega) =\begin{cases}
      \frac{\omega+2\pi}{2\pi},& -2\pi \leq \omega < 0\\
      \frac{2\pi-\omega}{2\pi},& 0 \leq \omega < 2\pi\\
      0,& else\\
    ...
    \(Y^*(j\omega) = |F(j\omega)|^2\), therefore we can say that \(Y(j\omega)\) is even and \(y(t)\) is real valued.\\
    \begin{math}
...
\end{center}
    ...
    \Rightarrow Z(j\omega) = \frac{1}{2\pi} \frac{1}{2} \left(rect(\frac{\omega+4\pi}{4\pi}) + rect(\frac{\omega-4\pi}{4\pi})\right) * rect(\frac{\omega}{4\pi}) \\
    \Rightarrow Z(j\omega) = \begin{cases}
        \frac{8\pi -\omega}{4\pi},& 4\pi< \omega < 8\pi \\
        \frac{\omega}{4\pi},&  0< \omega \leq 4\pi \\
        \frac{-\omega}{4\pi},&  -4\pi< \omega < 0 \\
       \frac{8\pi + \omega}{4\pi},&  -8\pi< \omega \leq -4\pi \\      
      \end{cases} 
    \end{math} 
    \begin{center}
  \begin{tikzpicture}
\begin{axis}[
    ...
    enlargelimits=true,
        ...
        \addplot [domain=-8*pi:-4*pi,style=very thick,black] {(8*pi+x)/(4*pi)};
        \addplot [domain=-4*pi:0,style=very thick,black] {(-x)/(4*pi)};
        \addplot [domain=0:4*pi,style=very thick,black] {(x)/(4*pi)};
        \addplot [domain=4*pi:8*pi,style=very thick,black] {(8*pi-x)/(4*pi)};
        \draw[dashed] (axis cs: -4*pi,0)--(axis cs: -4*pi,1);
        \draw[dashed] (axis cs: 4*pi,0)--(axis cs: 4*pi,1);
        \draw[dashed] (axis cs: -4*pi,1)--(axis cs: 4*pi,1);
\end{axis}
\end{tikzpicture}
\end{center}
    \subsubsection*{ii)}
    \begin{math}
    y(t) \longleftrightarrow Y(j\omega) = \frac{1}{2} rect(\frac{\omega+4\pi}{4\pi}) + \frac{1}{2} rect(\frac{\omega-4\pi}{4\pi}) \\
    Y(j\omega) = \frac{1}{2\pi} rect(\frac{\omega}{4\pi}) * \pi [\delta(\omega-4\pi) + \delta(\omega+4\pi)] \\
    \textrm{By modulation property of CTFT: } \\
    \mathscr{F}^{-1}\{Y(j\omega)\} = y(t) =  \frac{sin(2\pi t)}{\pi t} cos(4\pi t) 
    \end{math} 
    \section*{Question 4}
    \subsection*{a)}
    \subsubsection*{i)}
...
\end{center}
    ...
    \subsubsection*{v)}
    \(h[n]\) is not perodic since its DTFT, \(H(e^{j \Omega})\) does not consist of impulses. 
    \subsection*{c)}
    \subsubsection*{i)}
 ...
 \begin{math}
   ...
   v[n] = 3 (e^{j\frac{\pi}{6} n}+ e^{-j\frac{\pi}{6} n}) + 0 = 6cos(\frac{\pi}{6} n)
    ...
    w[n] = (-1)^{n} = \begin{cases}
      1,& n = 0, \pm2, \pm4, ... \\
      -1,& n = \pm1, \pm3, ...\\
      \end{cases} \Rightarrow w[n] = cos(\pi n) = \frac{1}{2} (e^{j\pi n} + e^{-j\pi n}) \\ 
      v[n] = \frac{1}{2} H(e^{j \pi}) e^{j\pi n} + \frac{1}{2} H(e^{-j \pi}) e^{-j\pi n} = \frac{1}{2} (e^{j\pi n} + e^{-j\pi n}) = cos(\pi n)
    \end{math}
\end{document}