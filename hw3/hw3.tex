...
\begin{axis}[
    ...
    enlargelimits=true,
        ...
        \addplot [domain=-6*pi:-2*pi,style=very thick,black] {1};
        \addplot [domain=-2*pi:2*pi,style=very thick,black] {0};
        \addplot [domain=2*pi:6*pi,style=very thick,black] {1};
        \draw[dashed] (axis cs: 2*pi,0)--(axis cs: 2*pi,1);
        \draw[dashed] (axis cs: -2*pi,0)--(axis cs: -2*pi,1);
        \draw[dashed] (axis cs: 6*pi,0)--(axis cs: 6*pi,1);
        \draw[dashed] (axis cs: -6*pi,0)--(axis cs: -6*pi,1);
        \draw[dashed] (axis cs: -2*pi,1)--(axis cs: 2*pi,1);
\end{axis}
\end{tikzpicture}
\end{center}
    \subsection*{b)}
